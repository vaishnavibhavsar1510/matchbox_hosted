...
export default function HostProfile() {
  ...
  useEffect(() => {
    if (userData) {
      setProfile({
        ...
        venueTypes: userData.venueTypes || [],
  ...
  return (
    <div className="min-h-screen bg-[#0A0F29]">
      <Head>
        <title>Host Profile - MatchBox</title>
        <meta name="description" content="Your MatchBox host profile" />
      </Head>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-[#1E1B2E] rounded-lg shadow-lg p-6 border border-purple-900/20">
          <h2 className="text-2xl font-semibold text-white mb-6">Host Profile</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label htmlFor="capacity" className="block text-sm font-medium text-purple-200 mb-2">
                ...
                <select
                  ...
                  className="w-full bg-purple-900/20 text-purple-200 placeholder-purple-400/50 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500/30 border border-purple-800/30"
                >
                  <option value="">Select typical capacity</option>
                  <option value="Small">Small (10-30 people)</option>
                  <option value="Medium">Medium (31-100 people)</option>
                  <option value="Large">Large (101-300 people)</option>
                  <option value="VeryLarge">Very Large (300+ people)</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-purple-200 mb-2">Event Types</label>
              <div className="flex flex-wrap gap-2">
                {profile.eventTypes.map((type, index) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newTypes = [...profile.eventTypes];
                        newTypes.splice(index, 1);
                        setProfile({ ...profile, eventTypes: newTypes });
                      }}
                ...
                <button
                  type="button"
                  onClick={() => {
                    const newType = prompt('Add a new event type:');
                    if (newType) {
                      setProfile({
                        ...profile,
                        eventTypes: [...profile.eventTypes, newType],
                      });
                    }
                  }}
            ...
            <div>
              <label className="block text-sm font-medium text-purple-200 mb-2">Venue Types</label>
              <div className="flex flex-wrap gap-2">
                {profile.venueTypes.map((type, index) => (
                  <span
                    key={index}
                    className="px-3 py-1 bg-purple-900/20 text-purple-200 rounded-full text-sm flex items-center border border-purple-800/20"
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newTypes = [...profile.venueTypes];
                        newTypes.splice(index, 1);
                        setProfile({ ...profile, venueTypes: newTypes });
                      }}
                      className="ml-2 text-purple-300 hover:text-purple-200 transition-colors duration-200"
                    >
                      ×
                ...
                <button
                  type="button"
                  onClick={() => {
                    const newType = prompt('Add a new venue type:');
                    if (newType) {
                      setProfile({
                        ...profile,
                        venueTypes: [...profile.venueTypes, newType],
                      });
                    }
                  }}
                  className="px-3 py-1 border border-purple-800/30 text-purple-300 rounded-full text-sm hover:bg-purple-900/20 transition-all duration-200"
